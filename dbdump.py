...
def dbdump_async(args,configfile=None):
    ...
    async def program(dbtype,host,user, password,server,port,include_databases,exclude_databases,structure_only_databases):
        ...
        try:
            ...
            results = await tqdm.asyncio.tqdm_asyncio.gather(*tasks, colour="white", bar_format="Eta:{eta}.|{bar}{r_bar}", desc="The progress of dumps", position=0, leave=True, return_exceptions=True)
            #aktresults = results
            #print(aktresults)
