        ...
        async with sem:
            try:
                ...
                for dumpcommand, mode in zip(dumpcommands, modes):
                    ...
                    if '-' in mode:
                        result = await conn.run(dumpcommand, stdout='%s/%s.sql.zst' % (sqlpath,mode), stderr='%s/%s-%s-%s-%s-%s-%s-%s.err' % (errpath,host,dbtype,server,port,database,mode,folderend[0:11]), check=True, encoding=None)
                    else:
